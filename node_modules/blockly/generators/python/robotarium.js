...
Blockly.Python['new_robot'] = function(block) {
  ...
  if (robot_color == '') robot_color = '"black"';
   var code = `
New_Robot("`+text_name+`",`+number_x_val+`,`+number_y_val+`,`+angle_theta+`,`+robot_color+`)
`;
   return code;
 };
...
Blockly.Python['run_to_point_and_angle'] = function(block) {
   ...
   var code = "np.append(target_Array, ["+number_target_x+","+number_target_y+","+angle+"])\n";
   return code;
 };
...
Blockly.Python['run_to_point'] = function(block) {
  var number_target_x = block.getFieldValue('target_x');
  var number_target_y = block.getFieldValue('target_y');
  var code = "Add_Target_Point(curName,"+number_target_x+","+number_target_y+",0)\n";
  return code;
};
...
Blockly.Python['turn_by_angle'] = function(block) {
  var angle_angle = block.getFieldValue('angle');
  var code = "robotList[namesList.index(curName)].turn("+angle_angle+")\n";
  return code;
};
...
Blockly.Python['move_forward'] = function(block) {
  ...
  var code = "robotList[namesList.index(curName)].move_Forward("+number_distance+")\n";
  return code;
};
...
Blockly.Python['turn_to'] = function(block) {
  var angle_angle = block.getFieldValue('angle');
  var code = "robotList[namesList.index(curName)].turn_To("+angle_angle+")\n";
  return code;
};

Blockly.Python['robot_do'] = function(block) {
  var robotName = block.getFieldValue('name');
  var robotCode = Blockly.Python.statementToCode(block, 'robotCode');
  var code = `
def `+robotName+` (self): {
    `+robotCode+` 
}

`+robotName+`(robotList[index('`+robotName+`')])\n`;
  return code;
};
...
class Robot:
    ...
    #   curX = self.target_points[self.totalPoints][0]
    #   curY = self.target_points[self.totalPoints][1]
    #   self.add_Target_Point((curX),(curY),(to))
...
while time.time() < t_end:
    ...
    dxu = unicycle_position_controller(x, target_Array(x))
